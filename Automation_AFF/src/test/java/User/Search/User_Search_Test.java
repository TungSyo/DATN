            ...
            for (int i = 1; i < rowCount; i++) {
                ...
                switch (action.toLowerCase()) {
                    ...
                    case "action":
                     ...
                     case "verifynotion":
                        Base_Action.handleVerification(searchActions.verifyNotion(result), "thông báo", result);
                        break;

                    case "verifytitle":
                        Base_Action.handleVerification(searchActions.verifyTitle(title), "tiêu đề", title);
                        break;

                    case "verifylink":
                        Base_Action.handleVerification(searchActions.verifyLink(link), "link", link);
                        break;
                    case "close":
