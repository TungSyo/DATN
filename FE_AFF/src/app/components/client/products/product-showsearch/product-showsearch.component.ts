...
@Component({
    ...
    templateUrl: './product-showsearch.component.html',
  styleUrls: ['./product-showsearch.component.scss'],
    providers: [MessageService],
})
export class ProductShowsearchComponent implements OnInit {
    cart: any[] = [];
    product: Product[] = [];
    ...
    this.route.queryParams.subscribe(params => {
      this.KeyWord = params['KeyWord'] ? params['KeyWord'].trim() : '';
      this.selectedCategoryId = params['CategoryId'] ? Number(params['CategoryId']) : null;
  
     
      this.searchProducts();
    //   if (this.KeyWord || this.selectedCategoryId) {
    //     this.searchProducts();
    //   }
    });
    }
    searchProducts() {
      const queryParams: any = {};  // Khởi tạo đối tượng queryParams trống
    
      // Kiểm tra và chỉ thêm các trường có giá trị vào queryParams
      ...
      // Các tham số phân trang và sắp xếp luôn luôn được thêm vào queryParams
      ...
      // Gọi API với queryParams đã được chuẩn hóa
      this.productService.getProductsT(queryParams).subscribe(
        (response) => {
    ...
    loadBestSellingProducts(pageIndex: number, categoryId?: number) {
      ...
      // Sử dụng SortBy và OrderBy nếu có
      if (this.sortBy !== null) {
        request.SortBy = this.sortBy;
      }
      request.OrderBy = this.orderBy;
  
      // Sử dụng từ khóa tìm kiếm nếu có
      if (this.KeyWord && this.KeyWord.trim() !== '') {
